...
type Category = "all" | "development" | "design" | "writing" | "translation";

// interface : 프롬프트 아이템 설계도
interface PromptItem {
    id: number;
    title: string;
    description: string;
    prompt: string;
    category: Exclude<Category, "all">; // 'all' 제외한 네 가지만
}

// interface : chat 세션 아이템 설계도
...
const promptData: PromptItem[] = [
    {
        ...
        title: "코드 리뷰어",
        description:
            "작성한 코드를 분석하고 품질, 가독성, 성능 측면에서 개선점을 제안해주는 프롬프트",
    ...
    },
    {
        ...
        title: "UI 피드백 분석가",
        description:
            "UI/UX 디자인 시안을 사용자 경험 관점에서 평가하고 구체적인 개선 방향을 제시해주는 프롬프트",
    ...
    },
    {
        ...
        title: "블로그 포스트 작성기",
        description:
            "주제와 타깃 독자를 입력하면 SEO를 고려한 구조적인 블로그 글을 작성해주는 프롬프트",
    ...
    },
    {
        ...
        title: "뉘앙스 살리는 번역가",
        description:
            "단순 직역이 아닌 문화적 맥락과 어감을 살려 자연스러운 번역을 제공해주는 프롬프트",
...
const chatSessionData: ChatSessionItem[] = [
    {
        chatSessionId: 1,
...
class ChatSessionManager {
    ...
    public render(): void {
        const filtered: ChatSessionItem[] = chatSessionData;
...
const promptGrid = document.getElementById("promptGrid") as HTMLDivElement;
const chatSessionGrid = document.getElementById(
    "chatSessionGrid",
) as HTMLDivElement;
...
const chatSessionsOpenBtn = document.getElementById(
    "chatSessionsOpenBtn",
) as HTMLButtonElement;
const chatSessionsCloseBtn = document.getElementById(
    "chatSessionsCloseBtn",
) as HTMLButtonElement;
const chatSessionsSidebar = document.getElementById(
    "chatSessionsSidebar",
) as HTMLElement;
const sidebarOverlay = document.getElementById(
    "sidebarOverlay",
) as HTMLDivElement;
const chatMessagesGrid = document.getElementById(
    "chatMessagesGrid",
) as HTMLDivElement;
...
//   const prompt = (document.getElementById("questionInput") as HTMLTextAreaElement).value;
  ...
  try {
    ...
    const botAnswerDiv = chatGrid.querySelector('.bot-answer') as HTMLElement;
    if (botAnswerDiv) {
...
function closeChatSession(): void {
...
filterBtns.forEach((btn) => {
    btn.addEventListener("click", function () {
        ...
        promptManager.setCategory((this.dataset.category as Category) || "all");
...
// 선택한 채팅의 상세 내용 가져오기
chatSessionGrid.addEventListener("click", function (e: MouseEvent) {
